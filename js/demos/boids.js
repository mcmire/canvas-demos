...
(function() {
  var BoidsCanvas, Boid, canvas

  BoidsCanvas = P(Canvas, function (proto, uber, klass, uberklass) {
    Boid = klass.Boid = P(Shape, function (proto, uber, klass) {
      klass.maxSpeed = 5
      return {
  ...
  canvas = window.canvas = BoidsCanvas("#wrapper", {
    width: 800,
    height: 600,
    fps: 30//,
    //trackFps: true,
  ...
  canvas.objects.addMany(60, Boid, function(i) {
    return {width: 8, height: 6, index: i}
  })
  $.v.extend(canvas.objects, {
    sepDistLimit: 50,
    sepVelLimit: 5,
    ...
    redraw: function() {
      ...
      $.v.each(this, function(boid) { boid.setVel() })
      $.v.each(this, function(boid) { boid.setPos(); boid.drawShape() })
    },
    ...
    // because when a boid is trying to figure out where the center is, though,
    // it's not going to include itself (since it can't see itself).
    //
    cohesionRule: function(boid) {
      var v, j
      v = Vec2(0,0)
      j = 0
      $.v.each(this, function(b) {
        if (b === boid) return
        Vec2.add(v, b.pos)
        j++
      })
    ...
    //
    // If we didn't have this here, the boids would collide with each other. In
    // order to prevent this, we simply look at each boid, and if it's within a
    // certain range of another boid, we start to move it as far away again as
    // it already is (with a smooth acceleration).
    //
    ...
    separationRule: function(boid) {
      ...
      $.v.each(this, function(b) {
        ...
        if (r1 < kr) {
          ...
          // This is just derived from the triangle similarity rule (all sides
          // of a similar triangle are in proportion).
          m = r2 / r1
          Vec2.sub(b.pos, boid.pos, v2)
          Vec2.mul(v2, m)
          // If there are multiple boids within proximity, I will back away from
          // their positions collectively
          Vec2.sub(v1, v2)
        }
      })
      return v1
    ...
    //
    // This is similar to Rule 1, however instead of averaging the positions of
    // the other boids we average the velocities. We calculate a 'perceived
